...
frame_step=1;
packet_step = 1;
margins = 1; % the number of frames to be removed from both sizes of every packet 
mode_2d = 1;
mode_lightcurve = 0;
mode_histogram = 0;
...
scurve_step = 1;
n_thr = 1100;
if(level==3) accumulation = 50000; end;

fix_color_map = 0;
  colorbar_lim = 35; %установить предел цветовой шкалы / set colorbar limit
...
%path='/home/alx/Downloads/20/';
%path='/home/alx/Downloads/16/'
...
for filename_cntr = 1:numel(listing) % указание на номера файлов, из которых будет произведено чтение
    ...
    if exist('D_unixtime_global', 'var')
        D_unixtime_global = [D_unixtime_global D_unixtime]
    else
        D_unixtime_global = D_unixtime;
    end;  

    continue;
         
    %lightcurve_sum=zeros(128);
    for i=1:packet_step:numel(sections_D(level,:))
        ...
        for current_frame=margins:frame_step:(num_of_frames-margins) % для каждого фрейма, прочитанного из файла / for each file in directory
